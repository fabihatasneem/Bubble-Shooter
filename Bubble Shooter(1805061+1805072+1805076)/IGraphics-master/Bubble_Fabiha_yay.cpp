...
void dfs(int x,int y)
{
    ...
    for(int i=0; i<4; i++)
    {
        ...
        if(u>0 && u<800 && v>0 && v<750)
        {
            if(vis[u][v]==1 && clr[u][v]==clr[x][y] && exist[u][v]==1)
            {
                dfs(u,v);
                if(cnt>=3)
                {
        ...
        for(int i=-20; i<=20; i++)
        {
            for(int j=0; j<=40; j++)
            {
                if(exist[x_shoot+i][y_shoot+j])
                {
                    ...
                    if(abs(j)>abs(i))
                    {
                        if(exist[x_shoot+i][y_shoot+j-20] && shoot_angle>0)
                        {
                            x_shoot+=i-40;
                            y_shoot=y_shoot+j;
                        }
                        else if(exist[x_shoot+i][y_shoot+j-20] && shoot_angle<0)
                        {
                            x_shoot+=i+40;
                            y_shoot=y_shoot+j;
                        }
...
void iDraw()
{
    ...
    else
    {
        ...
        {
            iLine(i,100,i+3,100);           //draw cute base
        }
        ...
        if(shoot_color==5)              //Special Ball 2 - vanish all bubbles of same color!
        {
            ...
            iSetColor(200,0,0);
            iFilledRectangle(x_shoot-15,y_shoot-5,10,10);

            iSetColor(0,0,200);
            iFilledRectangle(x_shoot+5,y_shoot-5,10,10);

            iSetColor(0,200,0);
            iFilledRectangle(x_shoot-5,y_shoot+5,10,10);

            iSetColor(200,200,0);
            iFilledRectangle(x_shoot-5,y_shoot-15,10,10);
        }
