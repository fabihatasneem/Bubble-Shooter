...
void iMouse(int button, int state, int mx, int my) // menu to game linkup and settings
{
    ...
    else if (!n && (highscore) && button == GLUT_LEFT_BUTTON &&(mx >= 650 && mx <= 750) && (my >= 100 && my <= 150))
    {
        if (state == GLUT_DOWN)
        {
            bk = 1;
    ...
    if (game_over && button == GLUT_LEFT_BUTTON &&(mx >= 650 && mx <= 750) && (my >= 100 && my <= 150))
    {
        if (state == GLUT_DOWN)
        {
            bk = 1;
