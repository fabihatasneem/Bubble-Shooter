...
int n = 0, p = 1, w1 = 0, w2 = 0, w3 = 0, w4 = 0, q1 = 0, q2 = 0, yes = 0, no = 0, bk = 0, opt = 0, no_sound = 0, pause = 0, resume = 0; // cover to game page linkup
int game_over = 0;
int score = 0;
    ...
    for(int i=0; i<4; i++)
    {
        ...
        if(u>0 && u<800 && v>0 && v<690)
        {
            if(vis[u][v]==0 && clr[u][v]==clr[x][y] && exist[u][v]==1)
            {
                ...
                {
                    if (!no_sound)
                        PlaySound("Pop.wav", NULL, SND_ASYNC | SND_ASYNC);
                    shooter_gone=1;
                    exist[u][v]=0;
                    score += 20;
                }
...
void iDraw()
{
    ...
    if (!n)
    {
        ...
        iShowBMP(0, 0, "wheel.bmp");
        iSetColor(0, 0, 0);
        iRectangle(260, 460, 280, 50);
        iText(330, 480, "NEW GAME", GLUT_BITMAP_TIMES_ROMAN_24);
        if (w1)
        {
            w2 = 0, w3 = 0;
            iSetColor(220, 20, 60);
            iFilledRectangle(260, 460, 280, 50);
            iSetColor(0, 0, 0);
            iText(330, 480, "NEW GAME", GLUT_BITMAP_TIMES_ROMAN_24);
        }
        iRectangle(260, 390, 280, 50);
        iText(315, 410, "HOW TO PLAY", GLUT_BITMAP_TIMES_ROMAN_24);
        if (w2)
        {
            w1 = 0, w3 = 0, w4 = 0;
            iSetColor(220, 20, 60);
            iFilledRectangle(260, 390, 280, 50);
            iSetColor(0, 0, 0);
            iText(315, 410, "HOW TO PLAY", GLUT_BITMAP_TIMES_ROMAN_24);
        }
        iRectangle(260, 320, 280, 50);
        iText(320, 340, "HIGH SCORES", GLUT_BITMAP_TIMES_ROMAN_24);
        if (w3)
        {
            w1 = 0, w2 = 0, w4 = 0;
            iSetColor(220, 20, 60);
            iFilledRectangle(260, 320, 280, 50);
            iSetColor(0, 0, 0);
            iText(320, 340, "HIGH SCORES", GLUT_BITMAP_TIMES_ROMAN_24);
        }
        iRectangle(260, 250, 280, 50);
        iText(340, 270, "OPTIONS", GLUT_BITMAP_TIMES_ROMAN_24);
        if (w4)
        {
            w1 = 0, w2 = 0, w3 = 0;
            iSetColor(220, 20, 60);
            iFilledRectangle(260, 250, 280, 50);
            iSetColor(0, 0, 0);
            iText(340, 270, "OPTIONS", GLUT_BITMAP_TIMES_ROMAN_24);
        }
    ...
    else
    {
        ...
        if(shoot_color==6)              //Special Ball 2 - vanish all bubbles of same color!
        {
            iSetColor(0,0,255);
            iFilledCircle(x_shoot, y_shoot, 20);
        ...
        if(rep_shoot_color==6)              //Special Ball 2 - vanish all bubbles of same color!
        {
            iSetColor(0,0,255);
            iFilledCircle(460,55, 15);
    ...
    {
        iPauseTimer(1);
        iShowBMP(0, 0, "wheel(3).bmp");
        iText(180, 420, "Your score: ", GLUT_BITMAP_TIMES_ROMAN_24);
    }
    if (!p)
    {
        //Fabiha touched Maneesha's code Lol. Now who's the winner muhahahahaha!!!!

        iShowBMP(0, 0, "wheel(4).bmp");
...
void iMouse(int button, int state, int mx, int my) // Maneesha's code. don't dare touch it
{
    if (!n)
        if (button == GLUT_LEFT_BUTTON && (mx >= 260 && mx <= 540) && (my >= 460 && my <= 510))
        {
            if (state == GLUT_DOWN)
                w1 = 1;
            else
        ...
        else if (button == GLUT_LEFT_BUTTON &&(mx >= 260 && mx <= 540) && (my >= 390 && my <= 440))
        {
            if (state == GLUT_DOWN)
                w2 = 1;
            else
        ...
        else if ((!p) && button == GLUT_LEFT_BUTTON &&(mx >= 650 && mx <= 750) && (my >= 100 && my <= 150))
        {
            if (state == GLUT_DOWN)
            {
                bk = 1;
        ...
        else if (button == GLUT_LEFT_BUTTON &&(mx >= 260 && mx <= 540) && (my >= 320 && my <= 370))
        {
            if (state == GLUT_DOWN)
                w3 = 1;
        }
        else if (button == GLUT_LEFT_BUTTON &&(mx >= 260 && mx <= 540) && (my >= 250 && my <= 300))
        {
            if (state == GLUT_DOWN)
                w4 = 1;
        }
        else if (button == GLUT_LEFT_BUTTON &&(mx >= 720 && mx <= 780) && (my >= 40 && my <= 100))
        {
            if (state == GLUT_DOWN)
                q1 = 1;
            else
    ...
    if ((q2) && button == GLUT_LEFT_BUTTON && (mx >= 220 && mx <= 320) && (my >= 320 && my <= 370))
    {
        if (state == GLUT_DOWN)
            yes = 1;
    ...
    else if ((q2) && button == GLUT_LEFT_BUTTON && (mx >= 480 && mx <= 580) && (my >= 320 && my <= 370))
    {
        if (state == GLUT_DOWN)
            no = 1;
        else if (state == GLUT_UP)
