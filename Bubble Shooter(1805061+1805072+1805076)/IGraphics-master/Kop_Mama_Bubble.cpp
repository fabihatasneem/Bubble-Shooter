...
int n = 0, p = 1, w1 = 0, w2 = 0, w3 = 0, w4 = 0, q1 = 0, q2 = 0, yes = 0, no = 0, bk = 0, opt = 0, no_sound = 0, pause = 0, resume = 0, option=0; // cover to game page linkup
int game_over = 0;
            ...
            for(int j=0; j<=40; j++)
            {
                if(exist[x_shoot+i][y_shoot+j]==1)
                {
                    ...
                    if(shoot_color==6)
                    {
                        for(int i=20; i<=780; i+=40)
                        {
                            for(int j=130; j<=y_shoot+80; j+=40)
                            {
...
void iDraw()
{
    ...
    if (game_over)
    {
        ...
        iText(400, 420, pscore, GLUT_BITMAP_TIMES_ROMAN_24);
    }
    if (!p)
    {
...
{
    if (!n && button == GLUT_LEFT_BUTTON && (mx >= 260 && mx <= 540) && (my >= 440 && my <= 490))
    {
        if (state == GLUT_DOWN)
            w1 = 1;
        else
